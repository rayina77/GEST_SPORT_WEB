...
@app.get("/clubs/add", response_class=HTMLResponse)
async def add_club_form(request: Request):
...
@app.post("/clubs/delete/{club_id}")
async def delete_club(club_id: int):

    conn = get_connection()

    # Détacher les athlètes du club
    conn.execute(
        """
        UPDATE athletes
        SET club_id = NULL
...
@app.get("/clubs/add",
         response_class=HTMLResponse)
async def club_add(request: Request):
...
@app.get("/ligues/new")
async def new_ligue(request: Request):

    return templates.TemplateResponse(
        request=request,
        name="ligue_form.html",
        context={
...
async def create_ligue(
    ...
    return RedirectResponse(
        url="/ligues?success=added",
        status_code=303
    )

@app.get("/ligues/edit/{ligue_id}")
async def edit_ligue(
        request: Request,
        ligue_id: int
):
    ...
    return templates.TemplateResponse(
        ...
        name="ligue_form.html",
        context={
            "title": "Modifier Ligue",
            "ligue": ligue
        }
    )

@app.post("/ligues/edit/{ligue_id}")
async def update_ligue(
    ...
    return RedirectResponse(
        url="/ligues?success=updated",
        status_code=303
    )

@app.post("/ligues/delete/{ligue_id}")
async def delete_ligue(ligue_id: int):
...
@app.get("/federations/new")
async def new_federation(request: Request):

    return templates.TemplateResponse(
        request=request,
        name="federation_form.html",
        context={
...
@app.get("/federations/edit/{federation_id}")
async def edit_federation(
        request: Request,
        federation_id: int
):
    ...
    return templates.TemplateResponse(
        ...
        name="federation_form.html",
        context={
            "title": "Modifier Fédération",
            "federation": federation
        }
    )


@app.post("/federations/edit/{federation_id}")
async def update_federation(
    ...
    return RedirectResponse(
        url="/federations?success=updated",
        status_code=303
    )
...
@app.post("/federations/delete/{federation_id}")
async def delete_federation(federation_id: int):
    ...
    return RedirectResponse(
        url="/federations?success=deleted",
        status_code=303
    )
